...
class GameFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnYes.setOnClickListener {
            onYesNoButtonClicked(binding.btnYes.text.toString())
        }

        binding.btnNo.setOnClickListener {
            onYesNoButtonClicked(binding.btnNo.text.toString())
        }

        startTimer()
    }

    private fun onYesNoButtonClicked(userAnswer: String) {
        ...
        if (!viewModel.nextWords(getString(R.string.easy))) {
            countDownTimer.cancel()
